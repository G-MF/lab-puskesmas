...
if (isset($_POST['tambah'])) {
    ...
    if ($cek) {
        ...
        $submit = $koneksi->query("INSERT INTO penerimaan VALUES(NULL, '$id_antri', '$tgl_penerimaan')");
    ...
    if (isset($_POST['edit'])) {
        ...
        $cek  = $koneksi->query("SELECT * FROM penerimaan p INNER JOIN nomor_antri n ON p.id_antri = n.id_antri INNER JOIN pasien pn ON n.id_pasien = pn.id_pasien WHERE p.id_penerimaan != '$id_penerimaan' AND tgl_penerimaan = '$tgl_penerimaan'")->fetch_array();
        if ($cek['id_antri'] == $id_antri) {
            $_SESSION['alert'] = 'Nomor Antrian Sudah Ada!';
            $_SESSION['valid'] = [
                'id_antri'   => $id_antri,
                'nama'       => $nama,
